...
for test_path in os.listdir(miltale_dir):
    if not test_path.endswith('pos'):
        continue
    test_path = miltale_dir + test_path
    if test_path in done:
        continue
    score = None
    pos_counts, avg_conf, per_unks, len_subwords, len_words = read_pos(test_path)
    
    lang = test_path.split('/')[-1].split('.')[0]
    if lang in conv:
        lang = conv[lang]
    swadesh_overlap, swadesh_correct = swadesh(test_path, lang)
    ...
    print(lang, swadesh_overlap, swadesh_correct, flush=True)
outFile.close()
