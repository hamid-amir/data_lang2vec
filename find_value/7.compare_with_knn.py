...
# do the same but using the unfiltered pos tags
print('If we use unfiltered pos tags:\n')
rf_values = list()
merge_values = list()
results = list()
for target_feature in myutils.target_features:
    with open(f'{save_dir}/unfiltered/optuna_study_results_method_{method}_target_feature_{target_feature}.json') as f:
        d = json.load(f)
        best_trial = max(d, key=lambda x:x['value'])
        rf_values.append(best_trial['value'])
        knn = knn_results[target_feature]
...
sorted_results = sorted(results, key=lambda x: x['knn'] - x["value"])
with open(f'result/knn_comparison_unfiltered.json', 'w') as f:
    f.write(json.dumps(sorted_results, indent=4))
